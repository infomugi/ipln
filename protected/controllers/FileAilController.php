...
class FileAilController extends Controller
{
	/**
	...
	public function actionCreate($id,$name)
	{
		...
		if(isset($_POST['FileAil']))
		{
			$model->attributes=$_POST['FileAil'];
			...
			if($model->save()){
				...
				// $ail = $this->loadAil($model->ail_id);
				// $ail->deskripsi = Yii::app()->user->name . " telah upload dokumen lampiran file ail pada ".date('d-m-Y h:i:s');
				// $ail->tanggal_update = date('Y-m-d h:i:s');
				// $ail->petugas_id = YII::app()->user->id;
				// $ail->save();				
	...
	{
		$dataProvider=new CActiveDataProvider('FileAil');
	...
	{
		$model=new FileAil('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['FileAil']))
			$model->attributes=$_GET['FileAil'];
	 ...
	 * @return FileAil the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=FileAil::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param FileAil $model the model to be validated
	 */

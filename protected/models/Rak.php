...
class Rak extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('kode, deskripsi, ruangan_id, status', 'required'),
			array('ruangan_id, status', 'numerical', 'integerOnly'=>true),
			array('kode', 'length', 'max'=>15),
			array('deskripsi', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_rak, kode, deskripsi, ruangan_id, status', 'safe', 'on'=>'search'),
			);
	}
